...
namespace :db do
  ...
  task :demo => :environment do
    ...
    puts "添加幼儿园"
    #添加幼儿园
    kind = Kindergarten.create!(:name=>"火龙果幼儿园",:number=>"huolg",:template_id=>template.id,:note=>"这是幼儿园简介",:weixin_token=>"weiyi",:weixin_status=>1,:sms_count=>1000,:sms_user_count=>50,:telephone=>"0755-86081688",:latlng=>"22.509512,113.939267",:address=>"幼儿园地址")
    kind.loading!
    puts "添加管理员"
    #添加管理员
    ...
    #添加6个学生
    (1..60).to_a.each do |i|
      user = User.create!(:weixin_code=>i == 1 ? "ofHgAj0UF75WoUvgUC-9dtccnYf4" : "weixin_code#{i}",:phone=>"139764719#{i / 10 == 0 ? ("0" + i.to_s) : i }",:name=>"学生#{i}",:note=>"我是学生#{i}",:tp=>0,:password=>"111111",:password_confirmation=>"111111",:kindergarten_id=>kind.id)
