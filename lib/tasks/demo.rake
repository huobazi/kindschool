...
namespace :db do
  ...
  task :demo => :environment do
    ...
    (1..60).to_a.each do |i|
      ...
      StudentInfo.create!(:card_code=>i == 1 ? "ofHgAj0UF75WoUvgUC-9dtccnYfQ" : "1000#{i}",:birthday=>Time.now,:squad_id=>kind.squads.random.id,:user_id=>user.id,:kindergarten_id=>kind.id)
      print "."
    end
    puts "demo添加完毕."
  end
