...
namespace :db do
  ...
  task :demo => :environment do
    ...
    puts "添加管理员"
    #添加管理员
    User.create(:login=>"huolg",:is_send=>true,:name=>"幼儿园管理员",:note=>"我是管理员",:tp=>2,:password=>"111111",:password_confirmation=>"111111",:kindergarten_id=>kind.id)
    puts "添加教职工"
    kind.reload
    #添加5个教职工
    (1..5).to_a.each do |i|
    ...
    #添加6个学生
    (1..60).to_a.each do |i|
      user = User.create(:weixin_code=>i == 1 ? "ofHgAj0UF75WoUvgUC-9dtccnYfQ" : "weixin_code#{i}",:login=>"student#{i}",:name=>"学生#{i}",:note=>"我是学生#{i}",:tp=>0,:password=>"111111",:password_confirmation=>"111111",:kindergarten_id=>kind.id)
