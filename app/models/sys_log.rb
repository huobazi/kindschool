...
class SysLog < ActiveRecord::Base
  ...
  def self.write_log(current_user_id,url,method,original_url,remote_ip,params,kind_id)
      ...
      if SYSLOGS[controller] && SYSLOGS[controller][action]
        ...
        if syslog = "发送消息"
         # if user = User.select(:name).find(params[:ids])#.size#.collect(&:name)#.collect(&name)
         # user_size=user.size
         # user_names = user[0...30].collect(&:name)
         # logtrail.node = "发送消息给#{user_size}人,#{params[:message][:tp]==1 ? '是' : '否'}发短信,#{params[:message][:send_me] ? '是' : '否'}发给自己,#{params[:message][:allsms]==1 ? '是' : '否'}发给所有人,发消息给#{user_names}(#{user_size>30 ? "后续省略" : "全部"})"
         # end
        end
        logtrail.save
      end
